...
class RevINTransformerBiLSTM(LightningModule):
    def __init__(
            self,
        ...
        self.Transformer_layer = Transformer(
            ...
            e_layers=1,  # encoder layer depth
        ...
        # LSTM layer
        self.lstm = nn.LSTM(input_size=self.input_size,  # The number of output channels from the Transformer is the input size for the LSTM
    ...
    def forward(self, input_seq: torch.Tensor):
        ...
        # Permute the dimension back to (batch_size, seq_len, input_size)
        normalized_input_seq = normalized_input_seq.permute(0, 2, 1)

        # Generate data for the Transformer layer
        enc_input = normalized_input_seq
        dec_zero = torch.zeros_like(normalized_input_seq[:, -self.num_days_for_predict:, :]).float()
        dec_input = torch.cat([normalized_input_seq, dec_zero], dim=1).float()
        mark_enc = None
        mark_dec = None
