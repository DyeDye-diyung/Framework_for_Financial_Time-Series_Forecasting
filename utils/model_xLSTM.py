...
from .xLSTM.xLSTM_Model import XlstmModel as xLSTM_layer
...
class xLSTM(LightningModule):
    def __init__(
        ...
        self.xlstm = xLSTM_layer(
            # sequence_len=self.num_days_for_predict,
            input_size=self.input_size,
            head_size=self.hidden_size,
            num_heads=self.num_directions,
            layers_str='sm',
        )
        # Full Connected layer
        self.linear = nn.Linear(self.input_size, self.num_days_to_predict)  # map input size to predicted horizon
    ...
    def forward(self, input_seq: torch.Tensor):
        ...
        # Pass through xLSTM layer
        xlstm_output = self.xlstm(input_seq)  # shape: (batch_size, seq_len, input_size)
        # Get the last time step output of the xLSTM
        xlstm_last_output = xlstm_output[:, -1:, :]  # shape: (batch_size, 1, input_size)

        # Permute input from (batch_size, 1, input_size) to (batch_size, input_size, 1)
        xlstm_last_output = xlstm_last_output.permute(0, 2, 1)  # shape: (batch_size, input_size, 1)

        # Pass through FC layer
        pred = self.linear(xlstm_last_output[:, :, -1])  # shape: (batch_size, seq_len_to_predict)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        y_in_and_pred = torch.cat([y[:, :self.num_days_for_predict, :], y_pred_reshape],
                                  axis=1)  # Concatenate with the original sequence to get (batch_size, seq_len_for_predict + seq_len_to_predict, out_var)
        loss = self.criterion(y_in_and_pred, y)
        # calculate RMSE
