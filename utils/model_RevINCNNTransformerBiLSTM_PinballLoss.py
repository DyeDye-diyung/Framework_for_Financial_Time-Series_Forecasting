...
def pinball_loss(y_pred: torch.Tensor, y_true: torch.Tensor, quantile: float = 0.5) -> torch.Tensor:
    """
    ?? Pinball Loss.
    """
    if not (0 < quantile < 1):
...
class RevINCNNTransformerBiLSTM(LightningModule):
    def __init__(
            self,
        ...
        self.Transformer_layer = Transformer(
            enc_in=self.cnn_out_channels,
            # The number of output channels from CNN is the input size for the Transformer
        ...
        self.lstm = nn.LSTM(input_size=self.cnn_out_channels,
                            # The number of output channels from the Transformer is the input size for the LSTM
                            hidden_size=self.hidden_size,
                            num_layers=self.num_layers,
                            batch_first=True,
                            bidirectional=True)  # Bidirectional LSTM
        # Fully connected layer
        self.linear = nn.Linear(self.hidden_size * self.num_layers,
    ...
    def forward(self, input_seq: torch.Tensor):
        ...
        Transformer_output = self.Transformer_layer(enc_input, mark_enc, dec_input,
                                                    mark_dec)  # shape: (batch_size, seq_len_for_predict, out_channels)
        ...
        lstm_output, hidden = self.lstm(Transformer_output,
                                        (h_0, c_0))  # shape: (batch_size, seq_len_for_predict, hidden_size)
        ...
        lstm_final_output = torch.cat([lstm_forward_last_output, lstm_backward_last_output],
                                      dim=2)  # shape: (batch_size, 1, hidden_size * 2)
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        loss = pinball_loss(y_in_and_pred, y, self.quantile)
