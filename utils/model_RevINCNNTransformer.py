...
class RevINCNNTransformer(LightningModule):
    def __init__(
            self,
    ...
    def forward(self, input_seq: torch.Tensor):
        ...
        Transformer_output = self.Transformer_layer(enc_input, mark_enc, dec_input,
                                                    mark_dec)  # shape: (batch_size, seq_len_to_predict, out_channels)

        # Take the variable of the last out_var dimensions
        target_output = Transformer_output[:, :, -self.out_var:]  # shape: (batch_size, seq_len_to_predict, 1)
