...
def config_parser(
	...
	GAN_parser.add_argument("--quantile", type=float, default=None,
							help="Quantile for Pinball Loss (only for PinballLoss models)")
	...
	for p in (GAN_parser, resume_parser):
		...
		trainer_parser.add_argument("--no-early-stop", action='store_false', dest="early_stop", help="Whether to use early stopping")
...
if __name__ == "__main__":
	...
	# torch.set_float32_matmul_precision('medium') # Enable Tensor Core acceleration on available GPU
	...
	pinball_loss_models = [
		'RevIN_CNN_iTransformer_BiLSTM_PinballLoss',
		'iTransformer_PinballLoss',
		'RevIN_CNN_Transformer_BiLSTM_PinballLoss',
		'Transformer_PinballLoss',
	]
	...
	# After parsing the parameters but before creating the Trainer, check the model type and modify the `monitor` property of the callback function accordingly
	if args.model_use in pinball_loss_models:
		print(f"Pinball Loss model ('{args.model_use}') detected.")
		print("Switching ModelCheckpoint and EarlyStopping to monitor 'val_loss'.")
		checkpoint_callback.monitor = 'val_loss'
		early_stop_callback.monitor = 'val_loss'
	else:
		# Make sure that for non-'Pinball Loss' models, what is monitored is 'val_RMSE'.
		print(f"Standard model ('{args.model_use}') detected.")
		print("ModelCheckpoint and EarlyStopping will monitor 'val_RMSE'.")
		checkpoint_callback.monitor = 'val_RMSE'
		early_stop_callback.monitor = 'val_loss'
	...
	if args.early_stop:
		# Update patience in early_stop_callback
		early_stop_callback.patience = args.early_stop_patience
		...
		case "new":
			...
			if args.model_use in pinball_loss_models:
				if args.quantile is None:
					parser.error(f"The model '{args.model_use}' requires the '--quantile' parameter, but not provided.")
				model_kwargs['quantile'] = args.quantile

			model = model_use(**model_kwargs)
